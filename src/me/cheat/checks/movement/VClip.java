...
extends Check {
    ...
    public VClip(Trojan trojan) {
        super("VClip", "VClip", trojan);
    ...
    public void onMove(PlayerMoveEvent event) {
        ...
        if (yDist < -0.5 || yDist > 0.5) {
            ...
            while (i < y) {
                Location l;
                 l = yDist < -0.5 ? to.clone().add(0.0, (double)i, 0.0) : from.clone().add(0.0, (double)i, 0.0);
        ...
        if (blocks > 0) {
            this.getTrojan().logCheat(this, player, "VClip", String.valueOf(blocks));
        }
    }
}
